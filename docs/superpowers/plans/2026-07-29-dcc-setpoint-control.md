...
- **Deceleration feels wrong at highway speed** → the decel side of the table is well-populated there and matches the measured floor, so suspect the `v_target` path or the min-speed clamp before suspecting the map.
- **Deceleration feels wrong at LOW speed (~30 km/h) under hard braking** → suspect the map first. The `v = 8.0 m/s` column at gaps −3.0/−2.0/−1.5 has only 95/56/64 real samples, below `MIN_CELL`, so those three cells are *not measured* — they are flat-extrapolated copies of the 50 km/h column (see `N_TABLE` in the generated module). This is the least trustworthy region of the table and the one to instrument first. Do not hand-edit the cells; gather low-speed braking data and regenerate.
