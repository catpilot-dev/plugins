#!/usr/bin/env python3
"""Scan plugin manifests and inject cereal schemas into custom.capnp and log.capnp."""
import glob
import json
import os
import re
...
def collect_cereal(plugins_dir: str) -> tuple[dict, list[tuple[str, str]]]:
  """Collect slot definitions and standalone schemas from enabled plugins.

  Returns (slots, standalone_files) where:
...
def main():
  ...
  if changes:
    print(f"[custom_capnp] Injected {changes} schema(s) into {cereal_dir}")
  else:
